/*
 * mc_cuda.cu - CUDA C implementation skeleton for Monte Carlo approximation
 * 
 * TODO: Implement Monte Carlo approximation for gravitational potential.
 * 
 ...
 * 
 * INSTRUCTIONS:
 * 1. Complete the compute_potential_mc_kernel function
 * 2. Use cuRAND to generate random particle indices
 * 3. Sample n_samples random particles for each target particle
 * 4. Scale the result appropriately
 * 
 * COMPILATION:
 *   nvcc -o mc_cuda mc_cuda.cu
 * 
 * HINTS:
 * - Initialize curandState with curand_init()
 * - Use curand() to generate random integers
 * - Avoid sampling the particle itself (i == j)
...
/*
 * CUDA kernel for Monte Carlo approximation of gravitational potential.
 * 
 ...
 *   n_samples   - Number of samples to take per particle
 *   G           - Gravitational constant
 *   seed        - Random seed for cuRAND
 */
__global__ void compute_potential_mc_kernel(
    ...
    unsigned long long seed)
{
    ...
    if (i < n_bodies) {
        ...
        // Sample n_samples random particles
        for (int k = 0; k < n_samples; k++) {
            // Generate random index j (avoiding i)
            int j;
            do {
                j = curand(&state) % n_bodies;
            } while (j == i);

            // Calculate distance to sampled particle
            float dx = xi - x[j];
            float dy = yi - y[j];
            float dz = zi - z[j];
        ...
        // Scale by total number and gravitational constant
        // The factor (n_bodies - 1) / n_samples scales the sample average to the full sum
        potentials[i] = G * pot * (float)(n_bodies - 1) / (float)n_samples;
    }
}

/*
 * Host function to compute potentials using Monte Carlo method.
 * This function is complete - no changes needed here.
 */
extern "C" void compute_potential_monte_carlo(
    ...
    unsigned long long seed)
{
    ...
    compute_potential_mc_kernel<<<blocks, threads_per_block>>>(
        d_x, d_y, d_z, d_masses, d_potentials, n_bodies, n_samples, G, seed);
...
int main(int argc, char **argv) {
    printf("CUDA Monte Carlo Approximation\n");
    printf("===============================\n\n");

    // Default parameters
    const char *pos_file = "plummer_data.bin";
    const char *output_file = "outputs/monte_carlo/potentials_mc_cuda.bin";
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--input") == 0 && i+1 < argc) {
            ...
            seed = (unsigned long long)atoi(argv[++i]);
        }
    }
    ...
    for (int i = 0; i < 3; i++) {
        compute_potential_monte_carlo(x, y, z, masses, potentials, n_bodies, n_samples, G, seed);
    }

    // Timed runs
    printf("Computing potentials using Monte Carlo (n_samples=%d) - 10 timed runs...\n", n_samples);
    float times[10];
    for (int i = 0; i < 10; i++) {
